...
public class ChefImpl implements Chef, BeltObserver {
	...
	private double spoiledFoodAmount;
	private double consumedFoodAmount;
		...
		if (e.getType() == BeltEvent.EventType.PLATE_CONSUMED) {
			...
			if (plate.getChef() == this) {
				...
				for (int i = 0; i < plate.getContents().getIngredients().length; i++) {
					consumedFoodAmount += plate.getContents().getIngredients()[i].getAmount();
				}
				//spoiledFoodAmount  consumedFoodAmount
			...
			if (plate.getChef() == this) {
				for (int i = 0; i < plate.getContents().getIngredients().length; i++) {
					spoiledFoodAmount += plate.getContents().getIngredients()[i].getAmount();
				}
	...
	@Override
	public double getFoodConsumedAmount() {
		// TODO Auto-generated method stub
		return consumedFoodAmount;
	}

	@Override
	public double getFoodSpoiledAmount() {
		// TODO Auto-generated method stub
		return spoiledFoodAmount;
	}
}
